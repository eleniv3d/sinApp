...
//Given the face normals
//worldX = new THREE.Vector(1 ,0 ,0)
//angle = normal.angleTo (worldX)
...
function curvatureApproximation(){
    ...
    geoBuf.geometry.center();
    var dict = {};

    for ( var i = 0; i < geoBuf.attributes.position.count; i += 3 ) {

        //create a dictionary of every position, and its neighboring positions
        var array = geoBuf.geometry.attributes.position.array;
        var normArray = geoBuf.geometry.attributes.normal.array;
    ...
    function filterBoth( curvatureFiltered );
    ...
    ninjaMeshRaw = new THREE.Mesh( geoBuf.geometry, materialRaw );

    scene.add( ninjaMeshRaw );


    function filterConvex() {
